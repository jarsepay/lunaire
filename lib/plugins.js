    ...
    try {
        ...
        function watchDirectory(directory) {
            fs.readdir(directory, (err, files) => {
                if (err) return console.log(colors(err, 'bgRed'))
                files.forEach(file => {
            ...
            try {
                ...
                if (eventType === 'rename') {
                    if (fs.existsSync(filePath)) {
                        ...
                        console.log(colors(`Plugin '${file}' added.`, 'green'))
                    } else {
                        delPlugin(file)
                        console.log(colors(`Plugin '${file}' removed.`, 'red'))
                    }
                } else if (eventType === 'change') {
                    await addPlugin(filePath, file)
                    console.log(colors(`Plugin '${file}' changed.`, 'cyan'))
                }
            } catch (e) {
                console.log(colors(`Plugin '${file}' error.`, 'bgRed'))
            }
        }
            ...
            for (const file of files) {
                ...
                if (fs.statSync(filePath).isDirectory()) {
                    ...
                    try {
                        ...
                        console.log(colors(`${e}: Plugin '${file}' error.`, 'bgRed'))
                    }
                }
            }
        }
